...
require('lazy').setup({
  ...
  { -- Opencode AI integration
    ...
    config = function()
      ...
      opencode.setup {
        -- Auto-connect to opencode running in current working directory
        auto_connect = true,

        -- Auto-reload buffers when opencode modifies them
        auto_reload = true,

        -- Show statusline indicator when connected
        statusline = {
          enabled = true,
        },

        -- Handle permission requests automatically for safe operations
        permissions = {
          auto_approve = {
            'read_file',
            'list_files',
      ...
      vim.keymap.set('n', '<leader>oa', function()
        opencode.ask()
      end, { desc = '[O]pencode [A]sk (interactive prompt)' })
      ...
      vim.keymap.set('n', '<leader>op', function()
        opencode.prompt()
      end, { desc = '[O]pencode [P]rompt (context-aware)' })

      vim.keymap.set('n', '<leader>oc', function()
        opencode.command()
      end, { desc = '[O]pencode [C]ommand' })

      -- Add current file to opencode context
      vim.keymap.set('n', '<leader>of', function()
        ...
        opencode.prompt { prompt = '@' .. filepath }
        vim.notify('Added file to Opencode: ' .. filepath, vim.log.levels.INFO)
      end, { desc = '[O]pencode add current [F]ile' })

      -- Add visual selection to opencode
      vim.keymap.set('v', '<leader>op', function()
        opencode.prompt()
      end, { desc = '[O]pencode [P]rompt with selection' })
    end,
  },
    ...
    config = function()
      require('oil').setup {
        ...
        keymaps = {
          ...
          ['_'] = 'actions.open_cwd',
          ['`'] = 'actions.cd',
          ['~'] = { 'actions.cd', opts = { scope = 'tab' }, desc = 'Set tab cwd' },
